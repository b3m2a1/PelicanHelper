...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated PelicanHelper loader file *)


Temp`PackageScope`PelicanHelperLoading`Private`$DependencyLoad=
  TrueQ@Temp`PackageScope`PelicanHelperLoading`Private`$DependencyLoad;
If[Temp`PackageScope`PelicanHelperLoading`Private`$DependencyLoad,
  Unprotect["`PelicanHelper`PackageScope`Private`Constants`$TopLevelLoad"];
  Evaluate[Symbol["`PelicanHelper`PackageScope`Private`Constants`$TopLevelLoad"]]=False,
  Unprotect["PelicanHelper`PackageScope`Private`Constants`$TopLevelLoad"];
  Evaluate[Symbol["PelicanHelper`PackageScope`Private`Constants`$TopLevelLoad"]]=
    MemberQ[$ContextPath, "Global`"]
  ];


If[Temp`PackageScope`PelicanHelperLoading`Private`$DependencyLoad,
  Evaluate[Symbol["`PelicanHelper`PackageScope`Private`$RootContext"]]=$Context;
  BeginPackage["`PelicanHelper`"],
  Evaluate[Symbol["PelicanHelper`PackageScope`Private`$RootContext"]]="";
  BeginPackage["PelicanHelper`"]
  ]


ClearAll[PelicanHelper];
PelicanHelper::usage="PelicanHelper is a manager head for the PelicanHelper package";
...
PelicanHelper["LoadingParameters"]:=$PackageLoadSpecs
$PackageLoadSpecs=
  Merge[
    {
      With[
...
PelicanHelper["Mode"]:=$PackageLoadingMode
$PackageLoadingMode=
...
PelicanHelper["Dependencies"]:=$PackageDependencies;
$PackageDependencies=
...
PelicanHelper["DependencyBase"]:=$PackageDependencyBase;
...
PelicanHelper["PackageListing"]:=$PackageListing;
$PackageListing=<||>;
PelicanHelper["Contexts"]:=$PackageContexts;
  ...
  $PackageContexts=
    Join[
      If[$PackageLoadingMode==="Dependency",
        ...
        {
          "PelicanHelper`",
          "PelicanHelper`PackageScope`Private`",
          "PelicanHelper`PackageScope`Package`"
          },
...
PelicanHelper["FEScopedSymbols"]:=$PackageFEHiddenSymbols;
$PackageFEHiddenSymbols={};
PelicanHelper["PackageScopedSymbols"]:=$PackageScopedSymbols;
...
PelicanHelper["FileContexts"]:=$PackageFileContexts;
...
PelicanHelper["DeclaredPackages"]:=$DeclaredPackages;
...
PelicanHelper["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
  With[{s=PackageFileContextPath[f]},
    StringRiffle[Append[""]@Prepend[s,PelicanHelper],"`"]
    ];
...
PelicanHelper::nodep="Couldn't load dependency `` of type ``";
PelicanHelper::nodup="Couldn't update dependency `` of type ``";
  ...
  Block[{retcode, site, path, file, tmp},
    ...
    site=
      ...
      If[retcode<300,
        ...
        DeleteFile[tmp];
        file,
        Message[PelicanHelper::nodep, dep, "Package"];
        DeleteFile[tmp];
        $Failed
        ]
      ];
    ...
    Monitor[
      MapThread[
        Check[
          ...
          Message[PelicanHelper::nodep, #2, "Paclet"];
          $Failed
          ]&,
        {
          pacs,
      ...
      Monitor[
        MapThread[
          Check[
            ...
            Message[PelicanHelper::nodup, #2, "Paclet"];
            $Failed
            ]&,
          {
            StringDelete[deps, "`"],
...
$PackageErrorMessage=
  "PelicanHelper encountered exception ``";
...
PackageCatchMessage[
  ...
  Replace[
    Thread[Map[Hold[MessageName[PelicanHelper, #]]&, msg], Hold],
    {
      {}:>
        Check[expr, 
...
PackageFailureException[
  ...
  (
    Set[MessageName[PelicanHelper, tag], body];
    PackageFailureException[
      MessageName[PelicanHelper, tag],
      body,
      ops
...
PackageAttachMainAutocomplete[]:=
  PackageAddAutocompletions[PelicanHelper, 
    Table[
      Replace[{}->None]@
        Cases[
          DownValues[PelicanHelper],
          Nest[
            Insert[#, _, {1, 1, 1}]&,
            (HoldPattern[Verbatim[HoldPattern]][
              PelicanHelper[s_String, ___]
              ]:>_),
            n-1
            ]:>s,
          Infinity
...
PackagePrepPackageSymbol[]:=
  Switch[$AllowPackageSymbolDefinitions,
    None,
      If[Length@OwnValues[PelicanHelper]==0,
        Remove[PelicanHelper],
        DownValues[PelicanHelper]={}
        ],
    False,
      If[Length@OwnValues[PelicanHelper]==0,
...
If[Temp`PackageScope`PelicanHelperLoading`Private`$DependencyLoad,
  If[
    (Clear@`PelicanHelper`PackageScope`Private`$loadAbort;!#)&@
      `PelicanHelper`PackageScope`Private`$loadAbort,
    Unprotect[`PelicanHelper`PackageScope`Private`$LoadCompleted];
    `PelicanHelper`PackageScope`Private`$LoadCompleted=True;
    `PelicanHelper`PackageScope`Private`PackagePostProcessContextPathReassign[]
    ],
  If[
    (Clear@PelicanHelper`PackageScope`Private`$loadAbort;!#)&@
      PelicanHelper`PackageScope`Private`$loadAbort,
...
Remove["Temp`PackageScope`PelicanHelperLoading`Private`*"]